<script lang="ts">
    ...
    const weatherMap: Record<number, { day: { description: string; image: string }, night: { description: string; image: string } }> = {
        0: {
    ...
    const iconInfo = weatherMap[code]?.[isDay ? "day" : "night"] ?? fallback;
</script>

<img
    src={`https://openweathermap.org/img/wn/${iconInfo.image}`}
    alt={iconInfo.description}
    title={iconInfo.description}
    class="w-16 h-16 inline-block align-middle"
/>
<span class="ml-2 align-middle text-gray-700">{iconInfo.description}</span>
